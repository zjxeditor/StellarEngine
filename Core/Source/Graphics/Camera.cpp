...
void BaseCamera::SetLookDirection(Vector3 forward, Vector3 up) {
	...
	forward = Select(forward * RecipSqrt(forwardLenSq), -Vector3(EZUnitVector::kZUnitVector), forwardLenSq < Scalar(0.000001f));

	// Deduce a valid, orthogonal right vector
	Vector3 right = Cross(forward, up);
	Scalar rightLenSq = LengthSquare(right);
	right = Select(right * RecipSqrt(rightLenSq), Quaternion(Vector3(EYUnitVector::kYUnitVector), -XM_PIDIV2) * forward, rightLenSq < Scalar(0.000001f));
