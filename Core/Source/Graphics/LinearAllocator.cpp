//
// Linear allocator for memory efficiency.
//
...
namespace Graphics
{
	...
	//
	// LinearAllocatorPageManager implementation.
	//
	...
	LinearAllocatorPageManager::LinearAllocatorPageManager()
	{
		...
		sm_AutoType = (LinearAllocatorType)((int)sm_AutoType + 1);
		ASSERT(sm_AutoType <= LinearAllocatorType::kNumAllocatorTypes);
	}
	...
	LinearAllocationPage* LinearAllocatorPageManager::RequestPage()
	{
		...
		while (!m_RetiredPages.empty() && Graphics::g_CommandManager.IsFenceComplete(m_RetiredPages.front().first))
		{
	...
	LinearAllocationPage* LinearAllocatorPageManager::CreateNewPage(size_t PageSize)
	{
		...
		if (m_AllocationType == LinearAllocatorType::kGpuExclusive)
		{
			...
			ResourceDesc.Width = PageSize == 0 ? (size_t)LinearAllocatorPageSize::kGpuAllocatorPageSize : PageSize;
		...
		else
		{
			...
			ResourceDesc.Width = PageSize == 0 ? (size_t)LinearAllocatorPageSize::kCpuAllocatorPageSize : PageSize;
	...
	void LinearAllocator::CleanupUsedPages(uint64_t FenceID)
	{
		...
		sm_PageManager[(int)m_AllocationType].DiscardPages(FenceID, m_RetiredPages);
		m_RetiredPages.clear();

		sm_PageManager[(int)m_AllocationType].FreeLargePages(FenceID, m_LargePageList);
		m_LargePageList.clear();
	}

	DynAlloc LinearAllocator::AllocateLargePage(size_t SizeInBytes)
	{
		LinearAllocationPage* OneOff = sm_PageManager[(int)m_AllocationType].CreateNewPage(SizeInBytes);
	...
	DynAlloc LinearAllocator::Allocate(size_t SizeInBytes, size_t Alignment)
	{
		...
		if (m_CurPage == nullptr)
		{
			m_CurPage = sm_PageManager[(int)m_AllocationType].RequestPage();
			m_CurOffset = 0;
		}
