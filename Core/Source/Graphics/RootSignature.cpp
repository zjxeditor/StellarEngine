//
// RootParameter and RootSignature utilities.
//
...
void RootSignature::Finalize(const std::wstring& name, D3D12_ROOT_SIGNATURE_FLAGS Flags) {
	...
	if (firstCompile) {
		...
		ASSERT_SUCCEEDED(Graphics::g_Device->CreateRootSignature(1, pOutBlob->GetBufferPointer(), pOutBlob->GetBufferSize(),
